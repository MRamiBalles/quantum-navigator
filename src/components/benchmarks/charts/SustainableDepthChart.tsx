...
export function SustainableDepthChart() {
  ...
  useEffect(() => {
    setIsLoading(true);
    fetch('/data/benchmark_sustainable_depth.json')
      .then(res => res.json())
      .then(json => {
        setData(json);
        setIsLoading(false);
      ...
      .catch(err => {
        ...
        setData(generateSustainableDepthData());
        setIsLoading(false);
      });
  }, []);
      ...
      <div className="h-80 relative">
        {isLoading ? (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-background/50 backdrop-blur-sm z-10 rounded-lg">
            ...
            <p className="text-sm font-medium animate-pulse">Modelando operación continua...</p>
          </div>
        ) : null}
